...
void fillTri(int x1, int y1, int x2, int y2)
{
    int i;
    ...
    setXY(x1, y1, x2, y2);
    int t = 2;
    for (i = y1; i < (y2 - y1) + y1 + 1; i++) {
     	setColor(50, 50, 100);
     	drawHLine(x1, i, (40-t)/2);
     	setColor(100, 0, 100);
     	drawHLine(x1+((40-t)/2), i, t);
     	setColor(50, 50, 100);
     	drawHLine(x1+((40-t)/2)+ t, i, (40-t)/2);
     	if(i%2){
     		t = t+2;
     	}
     }
...
void setBackground(){
	for(int x=0; x<201;x+=40){
		for(int y=0; y<281;y+=40){
			fillTri(x, y, x+40, y+40);
		}
	}
}

void setPartialBackground(){
	for(int x=0; x<201;x+=40){
		for(int y=80; y<161;y+=40){
			fillTri(x, y, x+40, y+40);
		}
	}
}

int setVolumeBackground(int edge){
	int x = 200;
	while(edge < x){
		x-=40;
	}
	fillTri(x, 120, x+40, 120+40);
	return x;
}
